...
if __name__ == '__main__':
    ...
    #print "hist", colors_hists.shape
    #print "seg", segments.shape
    #print "center", centers.shape
    #print "segment ids", np.unique(segments).shape
    #print "marking shape", img_marking.shape
    fgbg_superpixels = find_superpixels_under_marking(img_marking, segments)
    #print "fgseg", fgbg_superpixels[0].shape
    #print "bgseg", fgbg_superpixels[0].shape


    ######################################################################
    fg_hist = cumulative_histogram_for_superpixels(fgbg_superpixels[0], colors_hists)
    #fg_histNorm = normalize_histograms(fg_hist)
    bg_hist = cumulative_histogram_for_superpixels(fgbg_superpixels[1], colors_hists)
    #bg_histNorm = normalize_histograms(bg_hist)
    histNorm = normalize_histograms(colors_hists)
    fgbg_hists = [fg_hist, bg_hist]
    ##############################################################
    region = do_graph_cut(fgbg_hists, fgbg_superpixels, histNorm, neighbors)

    mask = pixels_for_segment_selection(segments, np.nonzero(region))
    mask = np.uint8(mask*255)
    #print "mask", mask[0:255,0]
    #cv2.imshow(mask)
    #cv2.cvtcolor(img_marking, cv2.color_bgr2gray) # dummy assignment for mask, change it to your result
    #master = cv2.imread("example_output.png", 0)
    #print "RMSD", RMSD(mask, master)
    # ======================================== #
